...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<PagedList<User>> GetUsers(UserParams userParams)
        {
            ...
            {
                var minimumDob = DateTime.Today.AddYears(-userParams.MaxAge - 1);
                var maximumDob = DateTime.Today.AddYears(-userParams.MinAge);

                users = users.Where(u => u.DateOfBirth >= minimumDob && u.DateOfBirth <= maximumDob);
            }
        ...
        private async Task<IEnumerable<int>> GetUserLikes(int id, bool likers)
        {
            ...
            if (likers)
            {
                return user.Likers.Where(u => u.LikeeId == id).Select(r => r.LikerId);
            }
            else
            {
